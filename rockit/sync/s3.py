...
def get_connection(**kwargs):
    ...
    if not access_key:
        ...
        if not access_key:
            raise EnvironmentError(
                "Missing config value for 'aws_access_key_id' or environment variable AWS_ACCESS_KEY_ID")
    ...
    if not secret_key:
        ...
        if not secret_key:
            raise EnvironmentError(
                "Missing config value for 'aws_secret_access_key' or environment variable AWS_SECRET_ACCESS_KEY")
...
def run_in_retry_loop(fn, max_retries=4, retry_sleep_time=default_retry_sleep_time):
    success = False
    retry = 0
    while not success:
        try:
...
def move_local_file_into_s3_dir(local_file, s3_path, make_public=True,
    ...
    if k.exists():
        log.warning("** already exists: %r, deleting: %r" % (key_name, local_file))
        return
